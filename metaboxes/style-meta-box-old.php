						...
						<div class="stylebox inner-style" tabindex="0">
							...
							<h3 class="design-name">
								<?php if ( $post->ID != $style->ID ) { ?>
									<a
										href="<?php echo esc_url( get_edit_post_link( $style->ID ) ); ?>"><?php echo esc_html( $style->post_title ); ?></a>
								<?php } else { ?>
							...
							<div class="theme-actions">
								...
								<a class="button button-primary customize load-customize hide-if-no-customize"
								   href="<?php echo esc_url( \Elementor\Utils::get_edit_link( $style->ID ) ); ?>"><?php esc_html_e( 'Edit', 'stylepress' ); ?></a>
							</div>
						</div>
					</li>
				<?php } ?>
