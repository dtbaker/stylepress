				...
				<div class="wp-clearfix">
					...
					<div class="stylepress-item-wrapper">
						...
						foreach ( $downloadable as $design_slug => $design ) :
							...
							?>
							<div class="design stylebox stylepress-<?php echo esc_attr( $type );
							if ( $has_purchased )
								echo ' stylepress-purchased' ?>" tabindex="0">
								...
								<div class="theme-actions">
									<a class="button" href="<?php echo esc_url( $design['demo'] ); ?>"
									   target="_blank"><?php esc_html_e( 'Preview', 'stylepress' ); ?></a>
									<?php if ( 'paid' === $type && ! $has_purchased ) { ?>
										<a class="button button-primary button-stylepress-pay"
										   href="<?php echo esc_url( wp_nonce_url( admin_url( 'admin.php?action=stylepress_download&slug=' . $design_slug ), 'stylepress_download', 'stylepress_download' ) ); ?>"
										   data-stylename="<?php echo esc_attr( $design['title'] ); ?>"
										   data-styleslug="<?php echo esc_attr( $design_slug ); ?>"
										   data-stylecost="<?php echo esc_attr( $design['cost'] ); ?>"><?php esc_html_e( 'Purchase', 'stylepress' ); ?></a>
									<?php } else { ?>
										<a class="button button-primary"
										   href="<?php echo esc_url( wp_nonce_url( admin_url( 'admin.php?action=stylepress_download&slug=' . $design_slug ), 'stylepress_download', 'stylepress_download' ) ); ?>"><?php esc_html_e( 'Install', 'stylepress' ); ?></a>
									<?php } ?>
								</div>
							</div>
						<?php endforeach; ?>
