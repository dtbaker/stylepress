...
<?php if ( $styles ) { ?>
	...
	<select name="dtbaker_style[style]" id="dtbaker_page_style">
		...
		<option
			value="-1"<?php echo $current_outer_style && (int) $current_outer_style === (int) - 1 ? ' selected' : ''; ?>><?php esc_html_e( 'Original Theme Output', 'stylepress' ) ?></option>
		<?php foreach ( $styles as $option_id => $option_val ) {
			?>
			<option
				value="<?php echo esc_attr( $option_id ); ?>"<?php echo $current_outer_style && (int) $current_outer_style === (int) $option_id ? ' selected' : ''; ?>><?php echo esc_attr( $option_val ); ?></option>
			<?php
		}
		?>
	</select>
...
if ( $components ) {
	...
	<select name="dtbaker_style[inner_style]" id="dtbaker_page_inner_style">
		...
		<option
			value="<?php echo STYLEPRESS_INNER_USE_PLAIN; ?>"<?php echo $current_inner_style && (int) $current_inner_style === (int) STYLEPRESS_INNER_USE_PLAIN ? ' selected' : ''; ?>><?php esc_html_e( 'Plain Output', 'stylepress' ) ?></option>
		<?php if ( $this->supports( 'theme-inner' ) ) { ?>
			<option
				value="<?php echo STYLEPRESS_INNER_USE_THEME; ?>"<?php echo $current_inner_style && (int) $current_inner_style === (int) STYLEPRESS_INNER_USE_THEME ? ' selected' : ''; ?>><?php esc_html_e( 'Use Theme Default Inner Output', 'stylepress' ) ?></option>
		...
		foreach ( $components as $option_id => $option_val ) {
			?>
			<option
				value="<?php echo esc_attr( $option_id ); ?>"<?php echo $current_inner_style && (int) $current_inner_style === (int) $option_id ? ' selected' : ''; ?>><?php echo esc_attr( $option_val ); ?></option>
			<?php
		}
		?>
	</select>
