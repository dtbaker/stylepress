...
function stylepress_register_google_maps_customize_control() {
	class stylepress_Google_Maps_Custom_Text_Control extends WP_Customize_Control {
		...
		public function render_content() {
			?>
			<p>Go to <a href="https://developers.google.com/maps/documentation/javascript/get-api-key" target="_blank">Google
					Maps API Page</a> and scroll down to the "Get a Key" button. Click this and follow the prompts. More details
				and step by step instructions are available in the theme documentation.</p>
			<?php
		}
	}
}
		...
		foreach ( $shortcode->fields as $field ) {
			...
			<p><label for="<?php echo $this->get_field_id( $field['name'] ); ?>"><?php echo $field['label']; ?>
					<?php switch ( $field['mce_type'] ) {
						case 'listbox':
							...
							<select name="<?php echo $this->get_field_name( $field['name'] ); ?>">
								<?php foreach ( $field['values'] as $key => $val ) { ?>
									<option
										value="<?php echo esc_attr( $key ); ?>"<?php echo $current_val == $key ? ' selected' : ''; ?>><?php echo $val; ?></option>
								<?php } ?>
							</select>
							<?php
							break;
						case 'textbox':
							?>
							<input type="text" name="<?php echo $this->get_field_name( $field['name'] ); ?>"
							       value="<?php echo esc_attr( isset( $instance[ $field['name'] ] ) ? $instance[ $field['name'] ] : $field['default'] ); ?>">
							<?php
							break;
					} ?>
				</label></p>
...
class stylepress_dtbaker_Shortcode_Google_Map {
	...
	public function dtbaker_shortcode_gmap( $atts = array(), $innercontent = '', $code = '' ) {
		...
		if ( ! $template_file ) {
			?>
			<div id="googlemap<?php echo (int) $map_id; ?>" class="googlemap"
			     style="height:<?php echo (int) $height; ?>px;"></div>
			<div class="clear"></div>
