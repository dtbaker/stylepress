			...
			<?php if ( $depth == 0 || $depth == 1 ) { ?>
				<p class="description description-wide">
					<label for="edit-menu-item-displaytype-<?php echo $item_id; ?>">
						...
						<select id="edit-menu-item-displaytype-<?php echo $item_id; ?>" class="widefat edit-menu-item-displaytype"
						        name="menu-item-displaytype[<?php echo $item_id; ?>]">
							<option value="">Default</option>
							<?php if ( $depth == 0 ) { ?>
								<option
									value="<?php echo STYLEPRESS_MENU_DISPLAY_MEGA; ?>" <?php selected( STYLEPRESS_MENU_DISPLAY_MEGA, $item->displaytype ); ?>><?php _e( 'Mega Menu' ); ?></option>
							<?php } ?>
							<option
								value="<?php echo STYLEPRESS_MENU_DISPLAY_SLIDEOUT; ?>" <?php selected( STYLEPRESS_MENU_DISPLAY_SLIDEOUT, $item->displaytype ); ?>><?php _e( 'Elementor Template' ); ?></option>
						</select>
					</label>
				</p>
				<p class="description description-thin stylepress-menu-display-slideout">
					<label for="edit-menu-item-slideout-<?php echo $item_id; ?>">
						...
						<select id="edit-menu-item-slideout-<?php echo $item_id; ?>" class="widefat edit-menu-item-slideout"
						        name="menu-item-slideout[<?php echo $item_id; ?>]">
							<option value="">None</option>
							<?php foreach ( $options as $template_id => $template_name ) { ?>
								<option
									value="<?php echo (int) $template_id; ?>" <?php selected( $template_id, $item->slideout ); ?>><?php echo esc_attr( $template_name ); ?></option>
							<?php } ?>
						</select>
				...
				<p class="description description-thin stylepress-menu-display-slideout">
					<label for="edit-menu-item-editslideout-<?php echo $item_id; ?>">
						...
						<a
							href="<?php echo esc_url( ! empty( $item->slideout ) ? \Elementor\Utils::get_edit_link( $item->slideout ) : admin_url( 'edit.php?post_type=elementor_library' ) ); ?>"
							target="_blank">Launch Elementor</a>
					</label>
				</p>
				<br style="clear:both">
				<p class="description description-wide stylepress-menu-display-slideout">
					<span class="description">Choose a slide out to appear <strong>instead of</strong> the normal drop down items below. You can create templates from the <a
							href="<?php echo esc_url( admin_url( 'edit.php?post_type=elementor_library' ) ); ?>" target="_blank">Elementor Library area</a>. The template will display for desktop users, mobile users will see a normal sub menu items as displayed below. Please make sure "Display Type" is set to "Elementor Template".</span>
				</p>
			<?php } ?>
			<!-- end stylepress -->
